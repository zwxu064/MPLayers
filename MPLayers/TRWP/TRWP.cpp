...
void ForwardCPU(const float rho,
  ...
  std::vector<float*> context_address(n_dir);
  ...
  for (uint iter = 0; iter < (uint)n_iter; ++iter) {
    for (uint dir = 0; dir < n_dir; ++dir) {
      if (dir < 2)
        Horizontal(param_list[dir], unary_ptr, msg_ptr, context_address[dir], edge_weight_ptr);

      if ((2 <= dir) && (dir < 4))
        DiagonalWide(param_list[dir], unary_ptr, msg_ptr, context_address[dir], edge_weight_ptr);
      ...
      if (4 <= dir) {
        ...
        if (h_step_abs > w_step_abs)
          DiagonalNarrow(param_list[dir], unary_ptr, msg_ptr, context_address[dir], edge_weight_ptr);
        else
          DiagonalWide(param_list[dir], unary_ptr, msg_ptr, context_address[dir], edge_weight_ptr);
      }
    }
...
void Forward(const float rho,
  ...
  if (unary.is_cuda()) {
    ...
    ForwardCUDA(rho, n_iter, (bool)enable_min_a_dir, unary, context, edge_weights,
                msg, cost_final, msg_min_index, msg_norm_index, unary_update,
                label_all);
#endif
  } else
